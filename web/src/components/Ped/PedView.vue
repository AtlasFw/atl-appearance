...
<template>
  ...
  <div class="w-full bg-slate-800 mt-3 rounded flex flex-col justify-evenly items-center">
    <div class="w-90% mt-3 mb-2 flex justify-between items-center">
      ...
      <span class="text-sky-200 font-medium">{{ $store.state.oldSkin['model'] }}</span>
    </div>
    <NSelect class="ml-2 mb-3 place-self-center mr-2.5 w-90%" @change="updateModel" v-model:value="$store.state.skin['model']" :consistent-menu-width="false" :options="$store.state.models"></NSelect>
  </div>
</template>